...
let process_pipeline _context s =
  Array.iter (fun s ->
    let m = of_msg s in
    match m.typ with
    | MapTask -> (
    ...
    | ShuffleTask -> (
      ...
      let _ = shuffle _context bar x z in
      barrier _context.myself_sock _context bar
...
let service_loop _context =
  ...
  try while true do
    ...
    | Pipeline -> (
      ...
      process_pipeline _context m.par;
      Utils.send ~bar _context.master_sock OK [||]
      )
    | Terminate -> (
